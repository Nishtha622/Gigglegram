...
export const bookmarkPosts = async (req, res) => {
	try {
		...
		const isAlreadyBookmarked = post.bookmarks.includes(userId);
		...
		if (isAlreadyBookmarked) {
			...
			return res.status(200).json({
				...
				bookmarks: post.bookmarks.filter(id => id.toString() !== userId.toString()),
			});
		} else {
			// Add the bookmark
		...
		console.error("Error in bookmarkPosts:", error);
		res.status(500).json({ error: "Internal server error" });
	}
}
...
export const generatePost = () => {
    return {
        ...
        imageUrl: faker.image.imageUrl(), // Generates a random image URL
        caption: faker.lorem.sentence(), // Generates a random caption
        likes: faker.datatype.number({ min: 0, max: 1000 }), // Random number of likes
        user: {
            username: faker.internet.userName(),
            profilePicture: faker.image.avatar() // Generates a random profile picture URL
        }
    };
};
